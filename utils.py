...
def load_images(source_dir):
    ...
    for (dirpath, dirnames, filenames) in os.walk(source_dir):
        imgs.extend([filename for filename in filenames if filenames.endswith('.jpg') or filenames.endswith('.png')])
        break
...
def parse(source_dir):
    """ Parse the input dir
    Args:
        source_dir: the dir has images and a 'image_list.txt'
    Returns:
...
def cylindrical_projection(img, focal_length):
    """ Project image to cylinder, to make better 360 image
    Args:
        img: input image
    ...
    for y in range(-int(h/2), int(h/2)):
        for x in range(-int(w/2), int(w/2)):
            ...
            if cylinder_x >= 0 and cylinder_x < w and cylinder_y >= 0 and cylinder_y < h:
